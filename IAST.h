...
public:
    ...
    virtual void appendColumnName(std::ostringstream &) const
    {
        throw Exception("Trying to get name of not a column: " + getID());
    }

    virtual void appendColumnNameWithoutAlias(std::ostringstream &) const
    {
        throw Exception("Trying to get name of not a column: " + getID());
    }
    ...
    void dumpTree(std::ostringstream & ostr, size_t indent = 0) const;
    ...
    struct FormatSettings
    {
        ...
        FormatSettings(std::ostringstream & ostr_, bool one_line_)
            : ostr(ostr_), one_line(one_line_)
        {
            nl_or_ws = one_line ? ' ' : '\n';
        }

        FormatSettings(std::ostringstream & ostr_, const FormatSettings & other)
