...
private:
    Tokens * tokens;
    BitmapExpressionTokens * bitmapTokens = nullptr;
    size_t index = 0;

public:
    explicit TokenIterator(Tokens & tokens_) : tokens(&tokens_) {}
    explicit TokenIterator(BitmapExpressionTokens & tokens_) : bitmapTokens(&tokens_) {}

    const Token & get()
    {
        if (bitmapTokens)
        {
            return (*bitmapTokens)[index];
        }
