...
public:
    ...
    // {
    //     throw Exception("Cannot convert NegativeInfinity to " + demangle(typeid(T).name()), ErrorCodes::CANNOT_CONVERT_TYPE);
    // }

    // T operator() (const PositiveInfinity &) const
    // {
    //     throw Exception("Cannot convert PositiveInfinity to " + demangle(typeid(T).name()), ErrorCodes::CANNOT_CONVERT_TYPE);
    // }
    ...
    T operator() (const String &) const
    {
        ...
        throw Exception("Cannot convert String to " + String(typeid(T).name()));
    }

    T operator() (const Array &) const
    {
        throw Exception("Cannot convert Array to " + String(typeid(T).name()));
    }

    T operator() (const Tuple &) const
    {
        throw Exception("Cannot convert Tuple to " + String(typeid(T).name()));
    }

    T operator() (const Map &) const
    {
        throw Exception("Cannot convert Map to " + String(typeid(T).name()));
    }
    ...
    T operator() (const Float64 & x) const
    {
        if constexpr (!std::is_floating_point_v<T>)
        {
            if (!isFinite(x))
            {
                ...
                throw Exception("Cannot convert infinite value to integer type");
            }
            else if (x > std::numeric_limits<T>::max() || x < std::numeric_limits<T>::lowest())
            {
                throw Exception("Cannot convert out of range floating point value to integer type");
            }
        }
    ...
    {
        throw Exception("Cannot convert AggregateFunctionStateData to " + String(typeid(T).name()));
    }
...
public:
    ...
    // bool operator() (NegativeInfinity &) const { throw Exception("Cannot sum -inf", ErrorCodes::LOGICAL_ERROR); }
    // bool operator() (PositiveInfinity &) const { throw Exception("Cannot sum +inf", ErrorCodes::LOGICAL_ERROR); }
    bool operator() (String &) const { throw Exception("Cannot sum Strings"); }
    bool operator() (Array &) const { throw Exception("Cannot sum Arrays"); }
    bool operator() (Tuple &) const { throw Exception("Cannot sum Tuples"); }
    bool operator() (Map &) const { throw Exception("Cannot sum Maps"); }
    // bool operator() (UUID &) const { throw Exception("Cannot sum UUIDs"); }
    bool operator() (AggregateFunctionStateData &) const { throw Exception("Cannot sum AggregateFunctionStates"); }
